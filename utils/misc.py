...
def model_load(cfg, net_G):
    ...
    elif cfg.CONST.pretrain:
        logger.info("Recovering Pretrain model from %s ..." % (cfg.CONST.weights))
        checkpoint = torch.load(cfg.CONST.pretrain)
        # 删除pretrained_dict中含有unet_encoder的item
        pretrained_dict = {}
        for k, v in checkpoint["net_G"].items():
            if "unet_encoder" not in k:
                pretrained_dict.update({k: v})
            else:
                pretrained_dict.update({k.replace("unet_encoder","unet_gan_encoder.unet_encoder"):v})

        # 如果修改了载入权重或载入权重的结构和当前模型的结构不完全相同，需要加strict=False，保证能够权重载入
        net_G.load_state_dict(pretrained_dict, strict=False)  # change into net_G!!
        model_dict = net_G.state_dict()

        # 冻结权重
        for name, para in net_G.named_parameters():
            # 不是unet_gan_encoder的层就冻结权重
            if "gan_generator" not in name:
                para.requires_grad = False

        logger.info("Recover Pretrain model complete.")
