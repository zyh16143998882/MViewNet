...
def print_table(
    ...
    for taxonomy_id in category_metrics:
        ...
        avg_temp = category_metrics[taxonomy_id].avg()
        for i in range(len(avg_temp)):
            print("%.4f" % avg_temp[i], end="\t")
            metrics_avg[i] += avg_temp[i]
        print()
    ...
    for i in range(len(metrics_avg)):
        metrics_avg[i] = metrics_avg[i] / len(category_metrics)
        print("%.4f" % metrics_avg[i], end="\t")
    print("\n")

    for i, m in enumerate(test_metrics.items):
        log_table["Avg" + "_" + m] = "%.6f" % metrics_avg[i]
