...
class inpaintingnetGANRunner(BaseRunner):
    ...
    def get_depth_image(self, data, code="default"):
        ...
        for _view_id in random_view_ids:
            partial_img, input_index = self.renderer_dis(data["partial_cloud"], view_id=_view_id, radius_list=[random_radius])
            ...
            if VISUALIZER_PRE == True:
                ...
                for i in range(img4.size()[0]):
                    vutils.save_image(img1[i,0,:,:,:], VIS_INPUT_PATH_POINT + '{}_{}.jpg'.format(str(code[i]), str(_view_id)),
                                      normalize=True)
                    vutils.save_image(img2[i,0,:,:,:], VIS_REAL_PATH_POINT+'{}_{}.jpg'.format(str(code[i]), str(_view_id)),
                                      normalize=True)         # torch.Size([1, 256, 256])
                    vutils.save_image(img3[i,:,:,:], VIS_PATH_PARTIAL + '{}_{}.jpg'.format(str(code[i]), str(_view_id)), normalize=True)
                    vutils.save_image(img4[i,:,:,:], VIS_PATH_GT + '{}_{}.jpg'.format(str(code[i]), str(_view_id)), normalize=True)
