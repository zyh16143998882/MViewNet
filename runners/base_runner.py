...
class BaseRunner(object):
    ...
    def __init__(self, config, logger):
        ...
        self.build_writer()
        self.build_dataset()
        self.build_models()     # netG的初始化
        self.data_parallel()
        self.models_load()      # 只有cfg.CONST.weights == True是才会load预训练模型
    ...
    def models_save(self):
        ...
        print("best_metrics == " + str(self.best_metrics))
    ...
    def train(self):
        ...
        for items in enumerate(self.train_loader):
            self.batch_idx, _ = items  # items由1个list组成,4个元素，最后一个是partial和gt的list
    ...
    def val_fake(self):
        ...
        for items in enumerate(self.fake_loader):
            self.model_idx, (taxonomy_id, _, model_id, data) = items
            self.taxonomy_id = (
                taxonomy_id[0]
                if isinstance(taxonomy_id[0], str)
    ...
    def val_finish(self):
        ...
        self.metrics = um.Metrics(self.config.TEST.metric_name, metrics_avg)
        self.models_save()
